...
class LevelAudioAnalyser(BaseAudioAnalyser):
    ...
    def __init__(self, source, callback, **opts):
        ...
        * The `error_callback` parameter should be a function
        ...
        self.__create_level_pipeline('low-pass', 100, 4, 1,
                                     lambda _, m: self.__on_message(
                                         self.MessageTag.LOW, _, m))
        self.__create_level_pipeline('high-pass', 5000, 4, 1,
                                     lambda _, m: self.__on_message(
                                         self.MessageTag.HIGH, _, m))
    ...
    def __create_level_pipeline(self, filter_mode, filter_cutoff, filter_poles,
                                filter_type, handler):
        elements = []
        ...
        audio_filter.set_property('mode', filter_mode)
        audio_filter.set_property('cutoff', filter_cutoff)
        audio_filter.set_property('poles', filter_poles)
        audio_filter.set_property('type', filter_type)

        # equ_bands = [[80, 160, 0], [10000, 9000, -24]]
        # audio_filter = Gst.ElementFactory.make('equalizer-nbands', None)
        # audio_filter.set_property('num-bands', len(equ_bands))
        # for band in equ_bands:
        #     audio_filter.get_child_by_index(0).set_property('freq', band[0])
        #     audio_filter.get_child_by_index(0).set_property('bandwidth', band[1])
        #     audio_filter.get_child_by_index(0).set_property('gain', band[2])
        elements.append(audio_filter)

        # Level messages.
        level_analyser = Gst.ElementFactory.make('level', None)
        level_analyser.set_property('interval', self.__interval * Gst.MSECOND)
        level_analyser.set_property('peak-falloff', self.__peak_falloff)
        level_analyser.set_property('peak-ttl', self.__peak_ttl * Gst.MSECOND)
        elements.append(level_analyser)

        # Sink
        sink = Gst.ElementFactory.make('fakesink', None)
        elements.append(sink)

        self._register_pipeline(elements, [('message::element', handler)])

        # Link
        link_ok = audio_source.link(caps_filter)
        link_ok = link_ok and caps_filter.link_filtered(audio_filter, caps)
        link_ok = link_ok and audio_filter.link(level_analyser)
        link_ok = link_ok and level_analyser.link(sink)
